...
function colorClasses(color) {
  const map = {
    ...
    amber: "text-[var(--brand-gold)] bg-[var(--brand-gold-soft)] border-[var(--brand-gold-border)]",
    emerald: "text-[var(--success)] bg-[var(--success-soft)] border-green-500/20",
    green: "text-green-600 bg-green-50 border-green-200",
    red: "text-[var(--danger)] bg-[var(--danger-soft)] border-red-500/20",
    purple: "text-[var(--brand-cyan)] bg-[var(--brand-cyan-soft)] border-[var(--brand-cyan-border)]",
  };
    ...
    <div className="rounded-3xl border border-red-500/20 bg-[var(--danger-soft)] p-6 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="font-semibold text-[var(--danger)]">Failed to load CRM</h3>
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-2xl border ${colorClasses(
                  kpi.color
                )}`}
              >
  ...
  return (
    <div className="rounded-3xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-sm">
      <div className="mb-5 flex items-center justify-between">
        <div>
          <h3 className="font-bold text-[var(--text-primary)]">Pipeline Snapshot</h3>
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-[var(--bg-card)] px-2 py-0.5 text-xs font-bold text-[var(--text-muted)]">
                    {stageItems.length}
                  </span>
                </div>
                ...
                <div className="mt-3 grid gap-2">
                  {stageItems.slice(0, 2).map((opp) => (
                    <button
                      ...
                      className="flex items-center justify-between rounded-2xl bg-[var(--bg-card)] px-3 py-2 text-left text-sm hover:bg-[var(--brand-cyan-soft)]"
                    >
        ...
        {opportunities.length === 0 ? (
          ...
          opportunities.map((opp) => (
            <button
              ...
              type="button"
              onClick={() => onOpportunityClick(opp)}
              className="w-full rounded-2xl border border-[var(--border-color)] bg-[var(--hover-bg)] p-4 text-left hover:bg-[var(--brand-cyan-soft)]"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-bold text-[var(--text-primary)]">{opp.name}</p>
                  <p className="mt-1 text-sm text-[var(--text-muted)]">{opp.company}</p>
                </div>
...
export function ClientRecentActivities({ activities }) {
  return (
    <div className="rounded-3xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-sm">
      <h3 className="font-bold text-[var(--text-primary)]">Recent Activities</h3>
      <p className="mt-1 text-sm text-[var(--text-muted)]">
...
export function ClientReportingSummary({ opportunities }) {
  ...
  return (
    <div className="rounded-3xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-sm">
      <h3 className="font-bold text-[var(--text-primary)]">Reporting Summary</h3>
      <p className="mt-1 text-sm text-[var(--text-muted)]">
...
export function ClientOpportunityPreviewDrawer({ opportunity, onClose }) {
  return (
    ...
    >
      <div
        className="h-full w-full max-w-md bg-[var(--bg-card)] p-6 shadow-2xl"
      ...
      >
        <div className="mb-6 flex items-start justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-2xl p-2 text-[var(--text-muted)] hover:bg-[var(--hover-bg)] hover:text-[var(--text-secondary)]"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
