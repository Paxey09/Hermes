  ...
  return (
    <div className="flex flex-col gap-4 border-b border-[var(--border-color)] pb-6 lg:flex-row lg:items-center lg:justify-between">
      ...
      <button
        type="button"
        onClick={onAddDeal}
        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[var(--brand-gold)] px-4 py-2.5 text-sm font-semibold text-[#050816] shadow-sm hover:bg-[var(--brand-gold-hover)]"
      >
...
export function ClientDealsLoadingState() {
  return (
    <div className="rounded-3xl border border-[var(--border-color)] bg-[var(--bg-card)] p-10 text-center shadow-sm">
      <p className="text-sm font-medium text-[var(--text-secondary)]">Loading deals...</p>
    </div>
  );
}
    ...
    <div className="rounded-3xl border border-red-500/20 bg-[var(--danger-soft)] p-6 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="font-semibold text-[var(--danger)]">Failed to load deals</h3>
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-[var(--border-color)] py-4 xl:flex-row">
      <div className="relative flex-1">
        ...
        <input
          className="h-11 w-full rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] pl-9 pr-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--brand-gold-border)] focus:ring-2 focus:ring-blue-100"
...
export function ClientDealsListView({
  ...
  if (deals.length === 0) {
    return (
      <div className="rounded-3xl border border-dashed border-[var(--border-color)] bg-[var(--bg-card)] p-10 text-center text-[var(--text-muted)]">
        No deals found.
      </div>
    );
  }
        ...
        <tbody className="divide-y divide-gray-100">
          {deals.map((deal) => (
            <tr key={deal.id} className="hover:bg-[var(--hover-bg)]">
              ...
              <td className="px-4 py-3 text-[var(--text-secondary)]">{deal.owner || "—"}</td>
              <td className="px-4 py-3">
                <DealStatusBadge status={deal.status} />
              </td>
              <td className="px-4 py-3">
                <button
                  ...
                  className="rounded-lg border border-[var(--border-color)] p-2 text-[var(--text-muted)] hover:bg-[var(--hover-bg)]"
                >
...
export function ClientDealsForecastView({
  ...
  return (
    <div className="rounded-3xl border border-[var(--border-color)] bg-[var(--bg-card)] p-6 shadow-sm">
      <h3 className="font-bold text-[var(--text-primary)]">Revenue Forecast by Stage</h3>
      ...
      <div className="mt-5 space-y-4">
        {byStage.map((row) => (
          ...
          >
            ...
            <span className="text-xs text-[var(--text-muted)]">{row.count} deals</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {rawStages.map((stage) => (
              <button
                ...
                className="rounded-lg border px-3 py-1 text-xs font-semibold transition hover:bg-[var(--hover-bg)]"
                style={{
                  borderColor:
                    stage.key === deal.stage ? stageColors[stage.key] : "var(--border-color)",
                  color:
                    stage.key === deal.stage ? stageColors[stage.key] : "var(--text-muted)",
                  background:
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-[var(--border-color)] bg-[var(--hover-bg)] p-4">
          <button
            type="button"
            onClick={() => onEdit?.(deal)}
