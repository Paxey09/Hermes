          ...
          <div key={card.label} className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)] p-5 shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-3 text-sm font-medium text-[var(--text-muted)]">Live contact data</p>
              </div>
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-[var(--border-color)] py-4 xl:flex-row">
      <div className="relative flex-1">
        ...
        <input
          className="h-11 w-full rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] pl-9 pr-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--brand-cyan)] focus:ring-2 focus:ring-[var(--brand-cyan-soft)]"
      ...
      <select className="h-11 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] px-3 text-sm text-[var(--text-secondary)]" value={filters.type} onChange={(e) => update("type", e.target.value)}>
      ...
      <select className="h-11 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] px-3 text-sm text-[var(--text-secondary)]" value={filters.status} onChange={(e) => update("status", e.target.value)}>
      ...
      <select className="h-11 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] px-3 text-sm text-[var(--text-secondary)]" value={filters.source} onChange={(e) => update("source", e.target.value)}>
      ...
      <select className="h-11 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] px-3 text-sm text-[var(--text-secondary)]" value={filters.sort} onChange={(e) => update("sort", e.target.value)}>
        ...
        <tbody className="divide-y divide-gray-100">
          {contacts.map((contact) => (
            <tr key={contact.id} className="hover:bg-[var(--hover-bg)]">
              ...
              <td className="px-4 py-3">
                <span className="rounded-xl bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">
              ...
              <td className="px-4 py-3">
                <div className="flex gap-2">
                  <button type="button" onClick={() => onRowClick(contact)} className="rounded-xl border border-[var(--border-color)] p-2 text-[var(--text-muted)] hover:bg-[var(--hover-bg)]" title="View">
                    <Eye className="h-4 w-4" />
                  </button>
                  <button type="button" onClick={() => onEditContact(contact)} className="rounded-xl border border-amber-200 bg-amber-50 px-2 text-sm text-amber-700 hover:bg-amber-100" title="Edit">
                    ✏️
                  </button>
                  <button type="button" onClick={() => onCreateDeal(contact)} className="rounded-xl border border-blue-200 bg-blue-50 px-2 text-sm text-blue-700 hover:bg-blue-100" title="Create Deal">
                    🎯
                  </button>
                </div>
              </td>
  ...
  return (
    <div className="space-y-3">
      {activities.map((activity) => (
        <div key={activity.id} className="rounded-xl border border-[var(--border-color)] bg-[var(--hover-bg)] p-4">
      ...
      <div className="h-full w-full max-w-xl bg-[var(--bg-card)] shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="border-b border-[var(--border-color)] bg-[var(--hover-bg)] p-6">
          <div className="flex items-start justify-between">
            ...
            <button type="button" onClick={onClose} className="rounded-xl p-2 text-[var(--text-muted)] hover:bg-gray-100">
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        <div className="h-[calc(100vh-220px)] overflow-y-auto p-6">
            ...
            <div className="space-y-3">
              {contact.related_deals?.length ? (
                contact.related_deals.map((deal) => (
                  <div key={deal.id} className="rounded-xl border border-[var(--border-color)] bg-[var(--hover-bg)] p-4">
                    <div className="flex items-center justify-between">
                      <div>
          ...
          {tab === "notes" && (
            <textarea
              className="min-h-32 w-full rounded-xl border border-[var(--border-color)] bg-[var(--hover-bg)] p-3 text-sm outline-none focus:border-[var(--brand-cyan)]"
        ...
        <div className="flex justify-end gap-2 border-t border-[var(--border-color)] bg-[var(--hover-bg)] p-4">
          <button type="button" onClick={() => onEdit?.(contact)} className="rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] px-4 py-2 text-sm font-semibold text-[var(--text-secondary)]">
            Edit
          </button>
