...
export default function AdminPredictive() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">Predictive Analytics</h1><p className="text-sm text-gray-500">AI-powered predictions based on historical data</p></div></div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Win Rate</p><p className="text-2xl font-bold">{predictions?.winRate || 0}%</p></CardContent></Card>
        <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Pipeline Value</p><p className="text-2xl font-bold">${(predictions?.totalPipeline || 0).toLocaleString()}</p></CardContent></Card>
        <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Forecast Next Month</p><p className="text-2xl font-bold text-blue-600">${(predictions?.forecastNextMonth || 0).toLocaleString()}</p></CardContent></Card>
        <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Forecast Next Quarter</p><p className="text-2xl font-bold text-purple-600">${(predictions?.forecastNextQuarter || 0).toLocaleString()}</p></CardContent></Card>
      </div>
    </div>
  );
}