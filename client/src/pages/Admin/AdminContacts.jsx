        ...
        <CardContent>
          ...
          {Object.keys(scoredContacts).length > 0 && (
            <div className="mt-4 space-y-2">
              <p className="text-sm font-medium">Contact Scores:</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {Object.entries(scoredContacts).map(([contactId, score]) => (
                  <div key={contactId} className="p-2 bg-[var(--bg-card)] dark:bg-[var(--bg-card)]/5 rounded text-sm">
                    <div className="flex justify-between">
                      <span className="font-medium">{filteredContacts.find(c => c.id === contactId)?.name?.slice(0, 20)}...</span>
                      <Badge variant={score.score > 70 ? 'success' : score.score > 40 ? 'warning' : 'default'}>
                        {score.score}
                      </Badge>
                    </div>
                    <p className="text-xs text-[var(--text-muted)] mt-1">{score.tier}</p>
                  </div>
                ))}
              </div>
            </div>
