...
async function getFacebookConfig(options = {}) {
  ...
  return {
    ...
    connectedWorkspaceId:
      ...
      (!isPageSpecificLookup ? fbRuntimeConfig.connectedWorkspaceId || "" : ""),
...
function saveRuntimeConfig(payload = {}) {
  ...
  if (typeof payload.connectedWorkspaceId === "string") {
    fbRuntimeConfig.connectedWorkspaceId = normalizeText(payload.connectedWorkspaceId);
  }
...
router.get("/client/inbox", async (req, res) => {
  ...
  const activePage =
    pages.find((page) => normalizePageId(page.pageId) === requestedPageId) || pages[0];
