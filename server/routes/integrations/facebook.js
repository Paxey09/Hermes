...
async function getFacebookConfig(options = {}) {
  ...
  return {
    ...
    connectedProfileName:
      supabaseConfig?.connectedProfileName ||
      (!isPageSpecificLookup ? fbRuntimeConfig.connectedProfileName || "" : ""),
...
function saveRuntimeConfig(payload = {}) {
  ...
  if (typeof payload.connectedProfileName === "string") {
    fbRuntimeConfig.connectedProfileName = normalizeText(payload.connectedProfileName);
  }
...
async function sendFacebookSenderAction(recipientId, action, context = {}) {
  ...
  if (!response.ok) {
    const details = await response.text();
    throw new Error(`Facebook sender action error (${response.status}): ${details}`);
  }
}
...
router.get("/client/pages", async (req, res) => {
  const profileName = normalizeText(req.query.profileName);
      ...
      try {
        ...
        try {
          await sendFacebookMessage(
            senderId,
            "I can only help with CRM, ERP, appointment booking, data analytics and market research, and email marketing.",
            { pageId }
          );
        } catch (fallbackError) {
