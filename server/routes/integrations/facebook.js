...
function isMessageOnTopic(userMessage = "", context = {}) {
  ...
  const outOfTopicPatterns = [
    ...
    /^(thanks|thank you|tq|thx)\s*$/i,
    /^(what|who|where|when|why)\s*$/i,
    /^\?+$/,
    /^!+$/,
    /^(joke|funny|laugh|lol|haha|hihi)/i,
    /^(weather|time|date|temperature|rain)/i,
    /^(music|song|movie|film|game|play|watch)/i,
    /^(recipe|cook|food|eat|drink|restaurant)/i,
    /^(travel|trip|hotel|flight|vacation|resort)/i,
    /^(news|sports|politics|celebrity|famous)/i,
    /^(love|relationship|dating|crush|boyfriend|girlfriend)/i,
    /^(homework|exam|study|test|school|university|grade)/i,
    /^(health|doctor|hospital|medicine|sick|pain)/i,
  ];
