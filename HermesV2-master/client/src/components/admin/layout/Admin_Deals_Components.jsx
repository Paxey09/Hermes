...
function statusBadgeClass(status) {
  if (status === "won") return "bg-emerald-50 text-emerald-700 border-emerald-200";
...
export function DealsKPICards({ deals }) {
  ...
  const cards = [
    { label: "Total Deals", value: total, icon: BriefcaseBusiness, color: "text-blue-600 bg-blue-50 border-blue-200" },
    { label: "Open Deals", value: open, icon: Target, color: "text-amber-600 bg-amber-50 border-amber-200" },
    { label: "Pipeline Value", value: formatShortCurrency(pipelineValue), icon: TrendingUp, color: "text-emerald-600 bg-emerald-50 border-emerald-200" },
    { label: "Avg Deal Size", value: formatShortCurrency(avgDealSize), icon: DollarSign, color: "text-purple-600 bg-purple-50 border-purple-200" },
    { label: "Closed Won", value: won, icon: Trophy, color: "text-green-600 bg-green-50 border-green-200" },
    { label: "Lost Deals", value: lost, icon: X, color: "text-red-600 bg-red-50 border-red-200" },
  ];
...
export function DealsFilterToolbar({
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-gray-200 py-4 xl:flex-row">
      ...
      <select className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm text-gray-700" value={filters.stage} onChange={(e) => update("stage", e.target.value)}>
      ...
      <select className="h-11 rounded-xl border border-gray-200 bg-white px-3 text-sm text-gray-700" value={filters.owner} onChange={(e) => update("owner", e.target.value)}>
...
function DealStatusBadge({ status }) {
  return (
    <span className={`rounded-full border px-2 py-1 text-xs font-bold uppercase ${statusBadgeClass(status)}`}>
      {status || "open"}
    </span>
  );
}
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-start gap-2">
          ...
          <div>
            <h4 className="text-sm font-bold text-gray-900">{deal.title || "Untitled Deal"}</h4>
            <p className="mt-1 text-sm font-medium text-blue-600">{deal.company || "No company"}</p>
            <p className="mt-1 text-xs text-gray-400">{deal.contact_name || "No contact"}</p>
          </div>
        </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <span className="font-bold text-emerald-600">{deal.probability || 0}%</span>
      </div>
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-6">
      {stages.map((stage) => {
        ...
        return (
          <div key={stage} className="min-h-[420px] rounded-2xl border border-gray-200 bg-gray-50 p-4">
        ...
        <tbody className="divide-y divide-gray-100">
          {deals.map((deal) => (
            <tr key={deal.id} className="hover:bg-gray-50">
              <td className="px-4 py-3 font-semibold text-gray-900">{deal.title || "Untitled Deal"}</td>
              <td className="px-4 py-3 text-blue-600">{deal.company || "—"}</td>
              ...
              <td className="px-4 py-3">
                <span className="rounded-full border px-2 py-1 text-xs font-bold" style={{ color: stageColors[deal.stage], borderColor: stageColors[deal.stage] }}>
                  {stageLabels[deal.stage] || labelize(deal.stage)}
                </span>
              </td>
              <td className="px-4 py-3 text-gray-600">{deal.probability || 0}%</td>
              <td className="px-4 py-3 text-gray-600">{deal.owner || "—"}</td>
              <td className="px-4 py-3"><DealStatusBadge status={deal.status} /></td>
              <td className="px-4 py-3">
                <button type="button" onClick={() => onRowClick(deal)} className="rounded-lg border border-gray-200 p-2 text-gray-500 hover:bg-gray-50">
...
export function DealsForecastView({ deals, stages, stageLabels, stageColors }) {
  const byStage = stages.map((stage) => {
    const stageDeals = deals.filter((deal) => deal.stage === stage && deal.status === "open");
    const total = stageDeals.reduce((sum, deal) => sum + Number(deal.value || 0), 0);
    const weighted = stageDeals.reduce((sum, deal) => sum + Number(deal.value || 0) * (Number(deal.probability || 0) / 100), 0);
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 space-y-4">
        {byStage.map((row) => (
          <div key={row.stage} className="grid gap-3 md:grid-cols-[140px_1fr_100px_120px_70px] md:items-center">
            ...
            <span className="text-sm font-bold text-gray-900">{formatShortCurrency(row.total)}</span>
            <span className="text-xs text-gray-500">Weighted: {formatShortCurrency(row.weighted)}</span>
            ...
            {rawStages.length > 0
              ...
              : Object.entries(stageLabels).map(([key, label]) => (
                  <button
                    ...
                    style={{
                      borderColor:
                        key === deal.stage ? stageColors[key] : "#e5e7eb",
                      color:
                        key === deal.stage ? stageColors[key] : "#9ca3af",
                      background:
                        key === deal.stage
                          ? `${stageColors[key]}15`
                          : "transparent",
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {tab === "overview" && (
            <div className="space-y-4 text-sm">
              ...
              <Info label="Expected Close" value={formatDate(deal.expected_close_date)} />
            </div>
          )}
